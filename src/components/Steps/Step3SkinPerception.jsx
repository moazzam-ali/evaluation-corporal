  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {options.map((key) => (
          <label key={key} className={`scan-radio ${value === key ? "is-checked" : ""}`} onClick={() => onChange(key)}>
            <input type="radio" checked={value === key} onChange={() => onChange(key)} className="sr-only" />
            <span className="scan-radio-dot" />
            <span>{t(`${optionPrefix}.${key}`, key)}</span>
          </label>
        ))}
      </div>
      {error && <p className="scan-error">{error.message}</p>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-[22px]">
      ...
      <RadioField
        ...
        t={t} error={errors.skinFeelGeneral}
      />
      <RadioField
        ...
        t={t} error={errors.skinFeelEndOfDay}
      />
    </div>
  );
}
