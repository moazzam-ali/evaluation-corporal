...
export default function ProductCard({ product, delay = 0 }) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -3, boxShadow: "0 14px 32px rgba(11,27,51,0.10)" }}
    >
      {/* Image area */}
      <div
        ...
        style={{
          ...
          background: isIngestible
            ...
            : "linear-gradient(160deg, #E6ECFF 0%, #B4C7FF 100%)",
      ...
      >
        ...
        {product.priority && (
          <span
            ...
            style={{
              padding: "5px 12px", background: "#0B1B33", color: "white",
        ...
        <span
          ...
          style={{
            ...
            color: isIngestible ? "white" : "#0B1B33",
      ...
      <div className="flex flex-col gap-3 flex-1" style={{ padding: "18px 20px 20px" }}>
        <div className="flex flex-col gap-0.5">
          <h3 style={{ fontFamily: "var(--font-dm-sans)", fontSize: "14.5px", fontWeight: 600, margin: 0, color: "#0B1B33", lineHeight: 1.3 }}>
            {product.name}
          </h3>
          <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: "11px", color: "#5A6B85" }}>
            {product.size}
          </span>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {topIngredients.map((ing, i) => (
              <button
                ...
                style={{
                  background: "#E3E8F0", color: "#0B1B33",
                  padding: "4px 10px", borderRadius: "999px",
                  fontFamily: "var(--font-dm-sans)", fontSize: "11px", fontWeight: 500,
                  border: "1px solid rgba(11,27,51,0.10)", cursor: "pointer",
                }}
              >
                <svg viewBox="0 0 24 24" width="10" height="10" fill="none" stroke="#2C5BFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 2v6L4 17a3 3 0 0 0 3 4h10a3 3 0 0 0 3-4l-5-9V2"/><line x1="9" y1="2" x2="15" y2="2"/></svg>
        ...
        <AnimatePresence>
          {showIngredients && product.keyIngredients?.length > 0 && (
            ...
            >
              <div className="space-y-1.5 rounded-xl p-3" style={{ background: "#F4F6FB" }}>
                {product.keyIngredients.slice(0, 4).map((ing, i) => (
                  <p key={i} style={{ fontSize: "11px", lineHeight: 1.5, color: "#5A6B85", margin: 0 }}>
                    <span style={{ fontWeight: 600, color: "#0B1B33" }}>{ing.name}</span>
                    <span style={{ margin: "0 4px", color: "rgba(11,27,51,0.16)" }}>—</span>
                    {ing.role}
                  </p>
                ))}
              </div>
        ...
        {product.howToUse && (
          <div
            ...
            style={{ background: "#F4F6FB", borderRadius: "10px", padding: "10px 12px" }}
          >
            <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="#2C5BFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            <span style={{ fontFamily: "var(--font-inter)", fontSize: "12px", color: "#5A6B85", lineHeight: 1.5 }}>
              {product.howToUse}
            </span>
          </div>
        )}
